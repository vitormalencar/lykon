...
const ActiveRoute = (to, pathname) => (to === pathname ? 'active-tab' : null)
...
const NavButton = ({ to, history, name, location }) => (
  <label onClick={() => onClickHandler(to, history)} className="tabbar__item">
    ...
    <button
      className={`tabbar__button tabbar--bottom-border__button ${ActiveRoute(
        to,
        location.pathname
      )}`}
    >
...
const Navigation = () => (
  <div className="tabbar">
    {routes
      ...
      .map((route) => (
        <NavButtonLink key={route.name} name={route.name} to={route.path} />
      ))}
  </div>
)
